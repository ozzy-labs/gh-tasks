...
- **--period** (任意): `daily` / `weekly` / `sprint` のいずれか。省略時は `daily`
...
2. ユーザーと優先順位を確認する
3. `gh tasks plan [--period ...] [--scope ...]` で確定する(repo scope は Milestone、org/user scope は Project v2 Iteration を更新)
...
> v0.1.0 スタブ。`gh tasks plan` の CLI 実装が完成するまで実用不可。
