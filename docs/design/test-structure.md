...
| `cmd/<cmd>_test.go` | cobra root を介さない pure helper テスト(`link_test.go` の URL parser、`plan_test.go` / `projects_test.go` の小さな utility 等)。 |
...
flow テスト全件が **同じ mock 表面** を共有する。共通フェイクは `internal/testfake/` に集約され、`cmd/testhelpers_test.go` がそれを薄くラップして cmd テスト固有の構文糖を提供する。

- `internal/testfake.GraphQL`: GraphQL 応答を `query <Name>(` の substring でマッチングする JSON-driven なキュー。`responses` を登録順に消費する。
  - 接頭辞かぶり (例 `ListRepoIssues` vs `ListRepoIssuesWithLabels`) を避けるため、必ず開きカッコ `(` まで含めた substring を指定する。
- `internal/testfake.RecordingREST`: REST 呼び出しを `<METHOD> <path-substring>` で照合し、応答を記録。`plan` の milestone 作成のように REST も叩く path で使う。
- `cmd/testhelpers_test.go` 内:
  - `captureGraphQL`: `testfake.GraphQL` をラップして outbound query / vars を覗く wrapper。フラグの伝播 (`--limit`, `--body` など) を assert したいときに使う。
...
4. 共通 helper / payload builder が必要
   ...
   - cmd / internal 双方で使う共通フェイク: `internal/testfake/` に追加
