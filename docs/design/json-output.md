...
| コマンド | Phase 1 (本シリーズ) | Phase 2 (別 issue) | 理由 |
| --- | :---: | :---: | --- |
...
| `done` / `link` / `plan --write` | — | ✅ | mutation 結果は exit code で十分なケースが多い |
| `projects init` / `init-templates` | — | ✅ | yaml が primary、JSON 化の優先度低 |

Phase 1 で 7 コマンド対応すれば、`gh tasks list --json` → script → `gh tasks add --json`(後続実行)の自動化ループが成立する。
...
- (Phase 2)スキーマ snapshot test: 各コマンドの `--json <all-fields>` 出力構造を golden file と比較し、不意の field 削除 / rename を検知
