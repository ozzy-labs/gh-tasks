...
`skills/{name}/SKILL.md` の frontmatter は以下のスキーマに従う。

```yaml
---
name: task-add
description: 会話文脈からタスクを追加する。scope に応じて GitHub Issue (repo) または Project draft item (org/user) を作成。
description_en: Capture a task from conversation context. Creates a GitHub Issue (repo) or Project draft item (org/user) per scope.
...
- frontmatter スキーマが `skills/` に閉じるため、本リポ固有要件を局所化できる
