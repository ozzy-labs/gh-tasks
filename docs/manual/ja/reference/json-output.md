...
カタログ本体は `cmd/jsonpath.go`(`itemJSONFields` / `activityJSONFields` / `linkJSONFields` / `projectInitJSONFields`)が SSOT。本ページの field 表は `<!-- begin: jsonout-catalog NAME -->` マーカーで囲まれており、in-source catalog から **in-place で再生成** される。pre-commit(lefthook)と CI(`go` job)が `check-json-schema --check` を呼び、drift があれば失敗する。

カタログを変更するときの手順:

1. `cmd/jsonpath.go` を編集(field の追加 / rename / 削除 / description / `Type` の更新)
2. *新規* catalog を追加した場合は `cmd/check_json_schema.go` の `jsonSchemaCatalogs()` にも登録し、本ページに `<!-- begin: jsonout-catalog NAME -->` / `<!-- end: jsonout-catalog NAME -->` の section を追加する(中身は空でよい — 次のステップが埋める)
3. 表を再生成:
...
4. `git add docs/manual/{en,ja}/reference/json-output.md` を `cmd/` の変更と一緒にコミット

ad-hoc に確認したい場合は `go run . check-json-schema`(フラグなし)で全カタログを stdout にダンプできる。CI gate は `--check` を使い、drift があれば exit 非 0。
