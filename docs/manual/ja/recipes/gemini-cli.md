...
Gemini CLI は `.gemini/settings.json` の `context.fileName` で指定したファイル(通常 `AGENTS.md`)を読み込む。Gemini CLI 自体には Claude Code のような `SKILL.md` 自動ロード機構が無いため、skill は AGENTS.md 内の marker block 経由で説明文として参照される。最短手順はワンショット install:

```bash
cd /path/to/your-repo
gh tasks install-skills            # `.gemini/` から gemini-cli を auto-detect
```

これで以下が atomic に実行される:

1. `.gemini/settings.json` を union merge で更新し、`{"context":{"fileName":["AGENTS.md"]}}` を注入。`model` / `temperature` 等の他キー、および `fileName` の他エントリは byte-for-byte で保護される。空 / 不在の場合は新規作成
2. `AGENTS.md` の marker block に gh-tasks skill 一覧を挿入。codex-cli adapter と同じ marker tag を共有するため、両方 install しても block は重複しない

主なバリエーション:

- `gh tasks install-skills --agent gemini-cli` — 明示指定
- `gh tasks install-skills --namespace gh-tasks` — 衝突回避用の rename install
- `gh tasks install-skills --uninstall` — manifest 記載のエントリを削除。AGENTS.md の marker は reference count(codex-cli が install されていれば残る)、`settings.json` からは AGENTS.md エントリのみ除去(`model` / `temperature` 等は保持)

skill の更新を Renovate 経路で取り込みたい場合は [`configs/skills-sync/README.md`](../../../../configs/skills-sync/README.md) を参照。

install 後の `.gemini/settings.json` 例:
...
- repo ルートで `gh tasks install-skills` を再実行すれば marker block と settings.json エントリの両方が更新される(idempotent)
