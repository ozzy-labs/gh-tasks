...
Codex CLI は `AGENTS.md` を起点として、参照される skill 本体を `.agents/skills/{name}/SKILL.md` から解決する。最短手順はワンショット install:
...
1. `.agents/skills/{name}/SKILL.md` を skill ごとに書き出す
2. 既存の `AGENTS.md` に marker block (`<!-- begin: @ozzylabs/gh-tasks -->` ～ `<!-- end: @ozzylabs/gh-tasks -->`) を idempotent に merge(marker block 外のコンテンツはバイト単位で保護される)
3. `.agents/skills/.gh-tasks-manifest.json` に provenance を記録

主なバリエーション:

- `gh tasks install-skills --agent codex-cli` — 明示指定
...
- `gh tasks install-skills --uninstall` — skill ファイルを削除。AGENTS.md の marker block は reference count されるため、gemini-cli も install されていれば marker は残る

skill の更新を Renovate 経路で取り込みたい場合は [`configs/skills-sync/README.md`](../../../../configs/skills-sync/README.md) を参照。両経路は配置先・marker tag・manifest を共有するため相互運用可能。
...
- repo ルートで `gh tasks install-skills` を再実行(idempotent — marker block と skill ファイルの両方を整える)
...
- `gh tasks install-skills` は marker 間を idempotent に書き換えて回復させる
